...
$title = "KRA SETTING";
$pagename = "incentive_setting_report.php";
...
<body class="bg-light">
    ...
    <div class="main w-auto">
        ...
        <div class="kra-wrapper">
            <div class="kra-page-title">Incentive Settings</div>

            <div class="kra-grid">

                <?php
                $types = [
                    'visit' => 'Average Visit / Beat',
                    'sales' => 'Average Sales / Beat (Lakh)',
                    'product_mix' => 'Product Mix / Beat',
                ...
                ?>
                    ...
                    <div class="kra-card">
                        ...
                        <div class="kra-card-body">
                            ...
                            <?php
                            $rows = $obj->executequery("SELECT * FROM incentive_slabs WHERE type='$key' ORDER BY min_value ASC");
                            foreach ($rows as $row):
                            ?>
                                <div class="slab-row">

                                    <input type="number" class="slab-input inc-range"
                                        value="<?= $row['min_value'] ?>"
                                        data-id="<?= $row['incentive_slabs_id'] ?>" data-type="min">
                                    ...
                                    <input type="number" class="slab-input inc-range"
                                        value="<?= $row['max_value'] ?>"
                                        placeholder="∞"
                                        data-id="<?= $row['incentive_slabs_id'] ?>" data-type="max">

                                    <input type="text" class="slab-input"
                                        value="<?= $row['amount'] ?>"
                                        data-id="<?= $row['incentive_slabs_id'] ?>" data-type="amount"
                                        placeholder="₹/beat">

                                    <button class="btn-del delete-inc" data-id="<?= $row['incentive_slabs_id'] ?>">✕</button>
    ...
    <script>
        ...
        $(document).on('change', '.inc-range', function() {
            ...
            $.post('update_incentive.php', {
                ...
                if (res === 'overlap') {
                    el.addClass('err');
                    toast('Overlap not allowed', true);
        ...
        $(document).on('change', '[data-type="amount"]', function() {
