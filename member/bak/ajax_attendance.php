...
function buildStateResponse($obj, $member_id, $meeting_id)
{
    ...
    if (!empty($sessions)) {
        ...
        if ($firstInTs > $shiftStartTs) {
            $lateSec = $firstInTs - $shiftStartTs;
        }
        ...
        if ($last['out_time'] !== null && $last['duration_sec'] > 0) {
            ...
            if ($lastOutTs < $shiftEndTs) {
                $earlySec = $shiftEndTs - $lastOutTs;
            }
        }
    }
...
if ($action === 'reset') {
    ...
    // Delete via executequery (multiple rows possible)
    $obj->executequery("
